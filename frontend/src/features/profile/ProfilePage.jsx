  ...
  async function saveSkills() {
    ...
    try {
      await usersApi.setup({
        skillsLearn: draftLearn,
        skillsTeach: draftTeach,
      });
      updateUser({ skillsLearn: draftLearn, skillsTeach: draftTeach });
      setEditingSkills(false);
      showToast("Skills updated! ✓");
    } catch (error) {
      showToast(error.message || "Failed to save skills");
    } finally {
    ...
    <main className="page-wrap feature-page">
      <header className="page-header">
        <div className="min-w-0">
          ...
          <p className="page-subtitle">
            Manage your skills and see your learning progress.
          </p>
        </div>
      </header>
      ...
      <section className="profile-grid">
        <article className="surface-card profile-summary-card">
          <div className="profile-identity">
            <div
              className="avatar profile-avatar text-white"
              style={{ background: user?.color || "#7657e8" }}
            >
              {user?.avatar}
            </div>
            <div className="min-w-0">
        ...
        <article className="surface-card profile-skills-card">
          ...
          <div className="skill-group">
            <p className="section-label">Want to learn</p>
            <div>
              {(user?.skillsLearn || []).map((skill) => (
                <span key={skill} className="tag tag-learn">
                  {skill}
                </span>
              ))}
              {!user?.skillsLearn?.length && (
                <span className="empty-copy">None added</span>
              )}
            </div>
          </div>
          <div className="skill-group">
            ...
            <div>
              {(user?.skillsTeach || []).map((skill) => (
                <span key={skill} className="tag tag-teach">
                  {skill}
                </span>
              ))}
      ...
      <section className="surface-card profile-progress-card">
        ...
        <div className="profile-progress-list">
          {COURSES.map((course) => (
            <div className="profile-progress-item" key={course.id}>
              <div>
      ...
      {editingSkills && (
        <div
          ...
          onClick={(event) => {
            if (event.target === event.currentTarget) setEditingSkills(false);
          }}
        >
          <div className="modal max-w-[620px]">
            <h3 className="modal-title">Edit skills</h3>
            <p className="modal-sub">
              Choose what you want to learn and what you can teach.
            </p>
            <div>
              <p className="section-label mb-3">Want to learn</p>
              <div className="filter-row">
                {SKILLS.map((skill) => (
                  <button
                    key={skill}
                    className={`filter-chip ${draftLearn.includes(skill) ? "filter-chip--active" : ""}`}
                    onClick={() => toggle("learn", skill)}
                  >
                    {skill}
                  </button>
                ))}
            ...
            <div>
              ...
              <div className="filter-row">
                {SKILLS.map((skill) => (
                  <button
                    key={skill}
                    className={`filter-chip ${draftTeach.includes(skill) ? "filter-chip--active-green" : ""}`}
                    onClick={() => toggle("teach", skill)}
                  >
                    {skill}
                  </button>
                ))}
